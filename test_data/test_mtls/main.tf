resource "aws_key_pair" "test" {
  public_key = "ssh-rsa  aleks@MediaPC"
}

module "frontend" {
  source = "../../"
  providers = {
    aws     = aws
    aws.dns = aws
  }
  service_name                 = "frontend"
  subnets                      = var.lb_subnet_ids
  ami                          = data.aws_ami.ubuntu.id
  backend_subnets              = var.backend_subnet_ids
  asg_name                     = var.asg_name
  asg_min_size                 = 1
  asg_max_size                 = 1
  internet_gateway_id          = var.internet_gateway_id
  zone_id                      = data.aws_route53_zone.website.zone_id
  dns_a_records                = var.dns_a_records
  key_pair_name                = aws_key_pair.test.key_name
  userdata                     = data.cloudinit_config.frontend_init.rendered
  health_check_type            = "ELB"
  tags                         = var.tags
  instance_profile_permissions = data.aws_iam_policy_document.webserver_permissions.json
  instance_role_name           = var.instance_role_name
  alb_access_log_enabled       = true
  alb_access_log_force_destroy = true
}

module "backend" {
  source = "../../"
  providers = {
    aws     = aws
    aws.dns = aws
  }
  service_name                 = "backend"
  subnets                      = var.lb_subnet_ids
  ami                          = data.aws_ami.ubuntu.id
  backend_subnets              = var.backend_subnet_ids
  asg_name                     = var.asg_name
  asg_min_size                 = 1
  asg_max_size                 = 1
  internet_gateway_id          = var.internet_gateway_id
  zone_id                      = data.aws_route53_zone.website.zone_id
  dns_a_records                = ["backend"]
  key_pair_name                = aws_key_pair.test.key_name
  userdata                     = data.cloudinit_config.backend_init.rendered
  health_check_type            = "ELB"
  tags                         = var.tags
  instance_profile_permissions = data.aws_iam_policy_document.webserver_permissions.json
  instance_role_name           = var.instance_role_name
  mtls_mode                    = "passthrough"
  # mtls_mode                    = "verify"
  alb_access_log_enabled       = true
  alb_access_log_force_destroy = true
  mtls_trust_store_arn         = module.truststore.truststore_arn
}
