...
- [ ] Create `alarms.tf` with SNS topic resource
- [ ] Add SNS email subscriptions to `alarms.tf`
- [ ] Add CloudWatch alarm for unhealthy host count
- [ ] Add CloudWatch alarm for target response time (latency)
- [ ] Add CloudWatch alarm for low success rate (5xx errors)
- [ ] Add CloudWatch alarm for high CPU utilization
- [ ] Add alarm variables to `variables.tf`
- [ ] Add alarm locals to `locals.tf`
- [ ] Add alarm outputs to `outputs.tf`
- [ ] Add warning check block to `deprecations.tf`
- [ ] Remove legacy CPU alarm from `cloudwatch.tf`

### Phase 3: Testing
- [ ] Extend `tests/test_create_lb.py` with alarm verification
- [ ] Test: SNS topic created with correct configuration
- [ ] Test: Email subscription exists (PendingConfirmation OK)
- [ ] Test: All 4 CloudWatch alarms created with correct thresholds
- [ ] Test: Alarm dimensions reference correct resources
...
- [ ] Create `alarms.tf` with SNS and CloudWatch resources
- [ ] Add variables to `variables.tf`
- [ ] Add locals to `locals.tf`
- [ ] Add outputs to `outputs.tf`
- [ ] Add warning check and validation check to `deprecations.tf`
- [ ] Extend `tests/test_create_lb.py` with alarm verification
- [ ] Update CHANGELOG.md
