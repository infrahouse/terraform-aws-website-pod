...
### 2. Remove `alb_healthcheck_enabled` Variable

**Status:** Planned for v6.0.0
**Issue:** https://github.com/infrahouse/terraform-aws-website-pod/issues/21
**Rationale:** Variable provides no value - AWS requires healthchecks enabled for instance-type target groups

#### Problem Statement

The `alb_healthcheck_enabled` configuration option is problematic because:

1. **AWS API Constraint:** For target groups with `target_type = "instance"`, AWS **requires** health checks to be enabled
2. **Invalid Configuration:** Setting to `false` triggers AWS error: "InvalidConfigurationRequest: Health check enabled must be true for target groups with target type 'instance'"
3. **No Practical Value:** A boolean that can only ever be `true` provides no configuration flexibility
4. **User Confusion:** Presence of the variable implies it can be disabled, leading to configuration errors
...
resource "aws_alb_target_group" "website" {
  # ...
  health_check {
    enabled             = var.alb_healthcheck_enabled  # Line 125
...
**After v6.0.0:**
```hcl
# Variable removed entirely from variables.tf

# main.tf - enabled attribute removed (AWS defaults to true)
resource "aws_alb_target_group" "website" {
  # ...
  health_check {
    # enabled attribute removed - AWS default is true for instance target groups
    path                = var.alb_healthcheck_path
    port                = var.alb_healthcheck_port
    # ...
  }
...
- **Prevents Invalid Configurations:** Eliminates possibility of setting `enabled = false` which always fails
- **Simplifies API:** Removes confusing variable that implies choice where none exists
- **Follows AWS Constraints:** Aligns module behavior with AWS API requirements
- **Cleaner Code:** Removes unnecessary configuration option
- **Better UX:** Users can't accidentally create invalid configurations

**Migration Impact:**

**Breaking Change:**
Users who explicitly set `alb_healthcheck_enabled` must remove it from their configuration.
...
module "website" {
  ...
  # All other healthcheck settings still work:
...
1. **User has `alb_healthcheck_enabled = true` (explicit):**
   - Remove the line
   - No functional change - healthchecks remain enabled

2. **User has `alb_healthcheck_enabled = false` (invalid):**
   - Configuration was already broken (AWS rejects it)
   - Remove the line
   - Healthchecks will be enabled (correct behavior)

3. **User doesn't set it (uses default = true):**
   - No change required
   - No functional change

**Implementation Steps:**

1. Remove `variable "alb_healthcheck_enabled"` from variables.tf (lines 27-31)
2. Remove `enabled = var.alb_healthcheck_enabled` from main.tf (line 125)
3. Update README.md via terraform-docs
4. Update UPGRADE-6.0.md with migration instructions
5. Update CHANGELOG.md
6. Review tests - remove any explicit `alb_healthcheck_enabled` settings

**Files to Update:**
- `variables.tf` (lines 27-31: remove variable definition)
- `main.tf` (line 125: remove enabled attribute from health_check block)
- `README.md` (terraform-docs auto-regenerates)
- `UPGRADE-6.0.md` (add migration section)
- `CHANGELOG.md` (document breaking change)
- `tests/*.py` (remove variable from test configurations if present)

**Testing Requirements:**
- Verify target group health_check works without explicit `enabled` attribute
- Confirm AWS defaults to `enabled = true` for instance target groups
- Test that healthcheck actually functions (instances become healthy)
- Verify terraform plan shows no changes after removing variable from user config

**Communication:**
In UPGRADE-6.0.md:
```markdown
### Removed `alb_healthcheck_enabled` Variable

**v5.x behavior:** Variable existed but could only be set to `true` (AWS constraint).
**v6.0.0 behavior:** Variable removed entirely.

**Action Required:**
Remove `alb_healthcheck_enabled` from your module configuration:
...
**Why this change:**
AWS requires health checks to be enabled for instance-type target groups.
Having a variable that can only be set to one value creates confusion and
allows invalid configurations. By removing it, we prevent configuration errors.

**Note:** Health checks remain fully functional - all other healthcheck variables
(path, interval, timeout, etc.) continue to work as before.
```

**Estimated Impact:**
- **Low adoption friction:** Simple variable removal
- **Low technical risk:** Removing unused/invalid option
- **High value:** Prevents invalid configurations, cleaner API

---

### 3. Change `alb_healthcheck_port` Type from `any` to `string`
