...
| <a name="input_alb_access_log_athena_enabled"></a> [alb\_access\_log\_athena\_enabled](#input\_alb\_access\_log\_athena\_enabled) | When true, creates the full Athena querying stack for this service's ALB<br/>access logs:<br/>- Glue catalog database and table (schema over the access log S3 bucket)<br/>- S3 results bucket (encrypted, 30-day expiry)<br/>- Athena workgroup pre-configured with the results bucket<br/><br/>The Glue database is named `<service_name>_<random_suffix>` (hyphens<br/>replaced with underscores) and the table is named<br/>`<service_name>_alb_access_logs`. | `bool` | `false` | no |
| <a name="input_alb_access_log_expiration_days"></a> [alb\_access\_log\_expiration\_days](#input\_alb\_access\_log\_expiration\_days) | Number of days to retain ALB access logs before expiration. | `number` | `365` | no |
...
| <a name="input_warm_pool_max_group_prepared_capacity"></a> [warm\_pool\_max\_group\_prepared\_capacity](#input\_warm\_pool\_max\_group\_prepared\_capacity) | Maximum total number of instances the ASG is allowed to maintain across<br/>InService + warm pool. Defaults to ASG max\_size when null. Only takes<br/>effect when warm\_pool\_state is set. | `number` | `null` | no |
| <a name="input_warm_pool_min_size"></a> [warm\_pool\_min\_size](#input\_warm\_pool\_min\_size) | Minimum number of instances to keep in the warm pool. Only takes effect<br/>when warm\_pool\_state is set. | `number` | `0` | no |
| <a name="input_warm_pool_reuse_on_scale_in"></a> [warm\_pool\_reuse\_on\_scale\_in](#input\_warm\_pool\_reuse\_on\_scale\_in) | Whether terminated instances should be reused (returned to the warm pool)<br/>on scale-in instead of being terminated. AWS default false. Only takes<br/>effect when warm\_pool\_state is set. | `bool` | `false` | no |
| <a name="input_warm_pool_state"></a> [warm\_pool\_state](#input\_warm\_pool\_state) | Pool state for the ASG warm pool. Set to enable a warm pool of<br/>pre-initialized instances that resume in seconds rather than running<br/>cloud-init from scratch on scale-out. Leave null to disable.<br/><br/>Valid values: "Stopped", "Running", "Hibernated".<br/>Hibernated is NOT supported on bare-metal instance types<br/>(e.g. c7i.metal-24xl) — use "Stopped" there. | `string` | `null` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_access_log_bucket_name"></a> [access\_log\_bucket\_name](#output\_access\_log\_bucket\_name) | Name of the ALB access log S3 bucket |
| <a name="output_access_log_replica_bucket_name"></a> [access\_log\_replica\_bucket\_name](#output\_access\_log\_replica\_bucket\_name) | Name of the access log replica bucket |
